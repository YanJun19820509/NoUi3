...
import { YJTweenAnimation } from '../base/ani/YJTweenAnimation';
...
@ccclass('SetPlayTweenAnimation')
@requireComponent(YJTweenAnimation)
export class SetPlayTweenAnimation extends HackUi {
    @property({ displayName: '播放所有动画' })
    playAll: boolean = false;

    protected onDataChange(data: any) {
        const comp = this.getComponent(YJTweenAnimation);
        if (this.playAll) {
            if (data === true) {
                comp?.a_playAll();
            } else if (data === false) {
