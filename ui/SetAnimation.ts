 ...
 *
 */
@ccclass('SetAnimationCb')
export class SetAnimationCb {
...
export class SetAnimation extends HackUi {
    ...
    /** 动画开始播放时触发 */
    private onPlay() {
        if (!this._curCb) return;
        no.EventHandlerInfo.execute(this._curCb.beforeStartHandlers);
    }

    /** 动画播放完成时触发（单次循环结束时也会触发） */
    private onFinished() {
        if (!this._curCb) return;
        no.EventHandlerInfo.execute(this._curCb.afterEndHandlers);
    }
    ...
    public onFrameEvent(v: any): void {
        if (!this._curCb) return;
        no.EventHandlerInfo.execute(this._curCb.eventHandlers, v);
    }
