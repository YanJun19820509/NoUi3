...
export class Simulator {
     ...
     */
    addAgent(position: { x: number, y: number }, properties?: { neighborDist?: number, maxNeighbors?: number, timeHorizon?: number, timeHorizonObst?: number, radius?: number, maxSpeed?: number, velocity?: { x: number, y: number } }) {
        ...
        agent.position_.reset(position.x, position.y);
        agent.maxNeighbors_ = properties?.maxNeighbors || this.defaultAgent.maxNeighbors_;
        agent.maxSpeed_ = properties?.maxSpeed || this.defaultAgent.maxSpeed_;
        agent.neighborDist = properties?.neighborDist || this.defaultAgent.neighborDist;
        agent.radius_ = properties?.radius || this.defaultAgent.radius_;
        agent.timeHorizon = properties?.timeHorizon || this.defaultAgent.timeHorizon;
        agent.timeHorizonObst = properties?.timeHorizonObst || this.defaultAgent.timeHorizonObst;
        const velocity = properties?.velocity || this.defaultAgent.velocity_;
        agent.velocity_.reset(velocity.x, velocity.y);
